...
const normalizeCommon = require('./common')
...
async function normalize (scenarioState, fixture) {
  ...
  fixture.headers = omitBy(fixture.headers, (value, key) => {
    // see https://github.com/octokit/fixtures/pull/20#issuecomment-331558385
    // see https://github.com/octokit/fixtures/pull/145#pullrequestreview-135886933
    return /^x-(now|zeit)-/.test(key) || key === 'now'
  })
  ...
  if (fixture.headers.link) {
    fixture.headers.link = fixture.headers.link
      .replace(/https:\/\/[^;]+/g, (url) => fixturizePath(scenarioState, url))
  }
